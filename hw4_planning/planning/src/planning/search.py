...
class ASTARPlanner(object):
    ...
    def Plan(self, start, goal):
        ...
        while len(queue) > 0:
            ...
            for neighbor, w in self.rm.graph[entry.node].items():
                ...
                ### BEGIN QUESTION 1.2 #####################
                neighbor_cost_to_come = entry.cost_to_come + weight
                neighbor_f_value = neighbor_cost_to_come  + h
                # ["f_value", "counter", "node", "parent", "cost_to_come"]
                queue.push(QueueEntry(neighbor_f_value, next(c), neighbor, entry.node, neighbor_cost_to_come))
    ...
    def extract_path(self, parents, goal):
        ...
        vpath = list()
        goal_state = parents[goal]
        vpath.append(goal)
        while (goal_state != NULL):
            vpath.insert(0, goal_state)
            goal_state = parents[goal_state]
        return vpath
...
class RRTPlanner(object):
    ...
    def Plan(self, start_config, goal_config, epsilon=0.001):
        ...
        for _ in range(self.max_iter):
            ...
            for i in range(xs_rand.shape[0]):
                ...
                # RRTTree -> GetNearestVertex
                near_x_id, x_near = self.tree.GetNearestVertex(x_rand)
                x_new = self.extend(x_near, x_rand)

                if x_new is not None:
                    vertex_id = self.tree.AddVertex(x_new) # add vertex to tree
                    self.tree.AddEdge(near_x_id, vertex_id) # add edge to tree
                    new_eid.append(vertex_id) # append its vertex id to the new_eid list
                    
                    # check by threshould
                    if self.prob.compute_distance(x_new, goal_config) < epsilon:
                        # stop iterations and set goal
                        found = True
                        goal_id = vertex_id 
    ...
    def ShortenPath(self, path):
        ...
        short_path = [path[0]]
        current = -1
        # iterate from goal node to start node
        while np.all(short_path[current] != [path[current]]):
            next = len(path) - 1
            while (next != 0):
                # check collision
                if self.prob.check_edge_validity(short_path[current], path[next]):
                    short_path.append(path[next])
                    break
                next -= 1
        return np.array(short_path)      
