...
class ModelPredictiveController(BaseController):
    ...
    def sample_controls(self):
        ...
        controls[:, :, 1] = alpha.reshape(self.K, 1)
    ...
    def get_rollout(self, pose, controls, dt=0.1):
        ...
        for i in range(self.T - 1):
                rollouts[:, i+1, :] = rollouts[:, i, :] + self.motion_model.compute_changes(
                rollouts[:, i, :], controls[:, i, :], dt, alpha_threshold=1e-2)
                # compute_changes(self, states, controls, dt, alpha_threshold=1e-2)  
    ...
    def compute_distance_cost(self, rollouts, reference_xyt):
        ...
        distance_cost = np.zeros(self.K)
        for i in range(self.K):
            distance_cost[i] = np.linalg.norm(rollouts[i, self.T-1, :2] - reference_xyt[:2])
        return distance_cost * self.error_w
        # END SOLUTION
    ...
    def compute_collision_cost(self, rollouts, _):
        ...
        sum = 0
        for i in range(self.K):
            sum = np.sum(rollout_collisions[i, :]) * self.collision_w
            costs[i] = sum
        return costs 
        # END SOLUTION
