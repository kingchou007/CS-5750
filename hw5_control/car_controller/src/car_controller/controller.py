...
def compute_position_in_frame(p, frame):
    ...
    # formula thetaF is frame[]
    F_p = np.dot(np.array([[np.cos(frame[2]), np.sin(frame[2])], 
...
class BaseController(object):
    ...
    def get_reference_index(self, pose, path_xytv, distance_lookahead):
        ...
        with self.path_condition:
            ...
            dis = np.linalg.norm(path_xytv[:, :2] - pose[:2], axis=1)
            min_index = np.argmin(dis)
            min = path_xytv[min_index, :2]
            for i in range(min_index, len(path_xytv) - 1):
                if np.linalg.norm(path_xytv[i, :2] - min) > distance_lookahead:
                    return i
