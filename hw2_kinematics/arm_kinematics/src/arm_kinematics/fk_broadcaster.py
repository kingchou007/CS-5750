...
class Foward_Kinematics_Broadcaster:
    ...
    def dh_transform(self, alpha, a, d, phi):
        T = np.matrix([[np.cos(phi), -np.sin(phi), 0, a],
    ...
    def compute_fk(self, q_waist, q_shoulder, q_elbow):
        ...
        shoulder_E_Uarm = self.dh_transform(np.pi/2, 0, 0, np.arctan(L3/L2) + np.pi/2 + q_shoulder)
        Uarm_E_Uforearm = self.dh_transform(0, np.sqrt(np.square(L2) + np.square(L3)), 
                                            0, np.arctan(L2/L3) + q_elbow)
