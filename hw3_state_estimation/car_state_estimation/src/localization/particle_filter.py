...
class ParticleInitializer:
    ...
    def reset_click_pose(self, msg, particles, weights):
        ...
        particles[:, 0] = np.random.normal(x, self.x_std, size = (n_particles,))
        particles[:, 1] = np.random.normal(y, self.y_std, size = (n_particles,))
        particles[:, 2] = np.random.normal(theta, self.theta_std, size = (n_particles,))
